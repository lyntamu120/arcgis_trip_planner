$(document).ready(function(){
    ...
    require([
        ...
        //params1.travelDirection = "From_FACILITY"
        //params1.travelModeName = "Driving Time"
        //end
        ...
        //params2.travelModeName = "Walking Time"
        ...
        map.on("load", function(evtObj) {
            ...
            params1.facilities.Cutoff_Walkingtime;
        ...
        function showLocation() {
            ...
            if (search1.searchResults != null && search2.searchResults != null) {
                if(search1.searchResults[0][0].name == 'Memorial Student Center'){
                    params1.defaultCutoff= 5.0;
                    params1.defaultTargetFacilityCount = 10;
                }else {
                    params1.defaultCutoff = 3.0;
                    params1.defaultTargetFacilityCount = 4;
                }
                if(search2.searchResults[0][0].name == 'Memorial Student Center'){
                    params2.defaultCutoff= 5.0;
                    params2.defaultTargetFacilityCount = 10;
                    params2.travelDirection = params1.travelDirection;
                }else{
                    params2.travelDirection = "From_FACILITY"
                    params2.defaultCutoff= 3.0;
                    params2.defaultTargetFacilityCount = 4;
                }
                console.log(search1.searchResults[0][0].name);
                ...
                closestFacilityTask.solve(params1, function(solveResult1){
                    ...
                    closestFacilityTask.solve(params2, function(solveResult2){
                        ...
                        /*console.log(points2);
                         ...
                         var candidate2 = [];
                         var n = k;
                         while(n >= k&&n < points2.length){
                         if(points2[n][2] == points2[k][2]){
                         candidate2.push(points2[n]);
                         }
                         n++;
                         }
                         break;
                         ...
                         candidate2.sort(function(a, b){
                         return a[3] - b[3];
                         });
                         for(var x = 0; x < candidate2.length; x++) {
                         if (x + 1 < candidate2.length && candidate2[x + 1][3] - candidate2[x][3] > 1) {
                         break;
                         }
                         }
                         var draw = false;
                         for(var y = 0; y <= x; y++){
                         for(var z = 0; z <points2.length; z++){
                         if(candidate2[y][0] == points2[z][0]&&candidate2[y][1] == points2[z][1]){
                         draw = true;
                         addGraphics(points2[z][2]);
                         addCurrentBuses(points2[z][2]);
                         addStops(points2[z][2]);
                         addPointsAndText(points2[z][2]);
                         var p1 = new Point(points1[j][0], points1[j][1], map.spatialReference);
                         var p2 = new Point(points2[z][0], points2[z][1], map.spatialReference);
                         var graphic1 = new Graphic(p1, symbolStart);
                         map.graphics.add(graphic1);
                         var graphic2 = new Graphic(p2, symbolEnd);
                         map.graphics.add(graphic2);
